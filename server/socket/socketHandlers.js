...
export const handleConnection = (io) => async (socket) => {
  ...
  // Update DB user status online on first connection
  ...
  // Send current online users to newly connected socket for state sync
  const onlineUsersArray = Array.from(connectedUsers.entries()).map(([uid]) => ({ userId: uid }))
  socket.emit('current_online_users', onlineUsersArray)

  // Handlers (join/leave conversation, send message, typing, mark read) remain unchanged
